...
fn stage_file_adds_to_index() {
    ...
    repo.add_and_commit("initial.txt", "initial", "initial commit").unwrap();
    ...
    let staged = changes.iter().find(|c| c.path == "new_file.txt" && c.staged);
...
fn stage_modified_file_updates_index() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "original", "initial commit").unwrap();
...
fn discard_file_changes_restores_original() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "original content", "initial commit").unwrap();
    ...
    // Verify unstaged change exists
    let changes = git_core::get_changes(&r).unwrap();
    assert!(changes.iter().any(|c| c.path == "file.txt"), "should have changes");
    ...
    let changes = git_core::get_changes(&r).unwrap();
    assert!(!changes.iter().any(|c| c.path == "file.txt"), "should have no changes");
...
fn get_changes_detects_untracked_files() {
    ...
    repo.write_file("untracked.txt", "untracked content").unwrap();
    ...
    assert!(!untracked.unwrap().staged, "untracked file should not be staged");
}

#[test]
fn get_changes_detects_deleted_files() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "content", "initial commit").unwrap();
    ...
    assert!(matches!(deleted.unwrap().status, git_core::ChangeStatus::Deleted));
}
...
fn diff_workdir_to_index_detects_unstaged_changes() {
    ...
    let file_diff = diff.files.iter().find(|f| f.new_path.as_ref().unwrap() == "file.txt");
...
fn diff_commits_shows_changes_between_commits() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "version 1", "commit 1").unwrap();
    repo.add_and_commit("file.txt", "version 2", "commit 2").unwrap();
...
fn diff_file_to_index_shows_single_file_changes() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "original", "initial commit").unwrap();
    repo.add_and_commit("other.txt", "other", "second commit").unwrap();
...
fn diff_refs_shows_changes_between_branches() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "main content", "main commit").unwrap();
    ...
    repo.add_and_commit("file.txt", "feature content", "feature commit").unwrap();
...
fn compute_inline_changes_detects_character_diffs() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "hello world", "initial commit").unwrap();
...
fn amend_commit_updates_message() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "content", "original message").unwrap();
...
fn validate_commit_ref_accepts_full_hash() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "content", "test commit").unwrap();
...
fn validate_commit_ref_accepts_branch_name() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "content", "main commit").unwrap();
...
fn create_commit_on_unborn_branch_works() {
    ...
    // Create first commit
    let commit_id = git_core::create_commit(&r, "first commit", "Test", "test@example.com").unwrap();
...
fn compute_graph_handles_merge_commits() {
    let repo = TestRepo::new().unwrap();
    repo.add_and_commit("file.txt", "main 1", "main commit 1").unwrap();
    repo.add_and_commit("file.txt", "main 2", "main commit 2").unwrap();
    ...
    std::process::Command::new("git")
        ...
        .output()
        .unwrap();
    repo.add_and_commit("feature.txt", "feature content", "feature commit").unwrap();
    ...
    assert!(merge_node.unwrap().parent_edges.len() > 1, "merge should have multiple parents");
}
...
fn compute_graph_assigns_increasing_lanes_for_branches() {
    ...
    std::process::Command::new("git")
        ...
        .output()
        .unwrap();
    repo.add_and_commit("feature.txt", "feature", "feature commit").unwrap();
    ...
    assert!(max_lane >= 1, "should have at least 2 lanes for divergent branches");
}
...
fn compute_ref_labels_includes_all_ref_types() {
    ...
    let has_local_branch = labels.iter().any(|l| matches!(l.ref_type, git_core::RefType::LocalBranch));
    let has_tag = labels.iter().any(|l| matches!(l.ref_type, git_core::RefType::Tag));
...
fn is_submodule_detects_submodule_path() {
    ...
    assert!(git_core::is_submodule(&r, "sub"), "should detect sub as submodule");
    assert!(!git_core::is_submodule(&r, "file.txt"), "should not detect regular file as submodule");
}
...
fn list_submodules_returns_empty_for_no_submodules() {
    ...
    assert!(submodules.is_empty(), "should return empty list when no submodules");
}
...
fn submodule_summary_returns_none_for_unchanged_submodule() {
    ...
    assert!(summary.is_none(), "should return None for unchanged submodule");
}
...
fn diff_between_same_commit_is_empty() {
    ...
    assert!(diff.files.is_empty(), "diff between same commit should be empty");
...
fn get_commit_with_invalid_id_fails() {
    ...
    assert!(result.is_err(), "getting commit with invalid id should fail");
}
...
fn amend_nonexistent_commit_fails() {
    ...
    let r = Repository::discover(repo.path()).unwrap();
    let result = git_core::amend_commit(&r, "0000000000000000000000000000000000000000", "new message");
